...
const isOAuthFlow = computed(() => {
  ...
  // 国产供应商（kimi / zhipu / deepseek）仅支持 API Key，不需要 OAuth 流程
...
watch(
  ...
  ([category, method, agType]) => {
    ...
    // 国产供应商（kimi / zhipu / deepseek）仅支持 API Key
    if (isCNPlatform.value) {
      form.type = 'apikey'
      return
    }
...
const handleSubmit = async () => {
  ...
  // 注意：国产平台（kimi / zhipu / deepseek）的 base_url 由下方 CN 区块按
  // account_mode + api_protocol 重新写入，这里只兜底非 CN 平台。
