...
vi.mock('@/stores', () => ({
  ...
  // 本地 fork 的 RegisterView 多读一个 merchantStore（商户分站隐藏优惠码框），
  // 上游用例按上游组件写的 mock 没有它，缺了会在 setup 阶段直接抛错。
  // 这里按主站语义给 false，与上游用例断言的非商户站场景一致。
  useMerchantStore: () => ({ isMerchantSite: false })
}))
