...
function declineLoginAgreement(): void {
  agreementAccepted.value = false
  showAgreementModal.value = false
  appStore.showWarning(t('auth.registerAgreement.declineHint'))
}
...
async function handleOAuthStart(request: OAuthLoginStart): Promise<void> {
  // 本地定制：OAuth 一旦启动就会跳走，必须先勾过注册协议（oauthActionDisabled 已含协议门禁）。
  // 上游此处只判 registrationActionDisabled，跳转从子组件上移到本处后需补回协议门禁。
  if (oauthActionDisabled.value) return
