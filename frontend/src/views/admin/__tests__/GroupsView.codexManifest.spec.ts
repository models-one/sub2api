...
// GroupsView 的 setup 里还调了 useAuthStore()，上游这份用例漏了这个 mock，
// 于是 pinia 未激活直接抛 "getActivePinia() was called but there was no active Pinia"。
// 该用例不在 CI 的 critical 列表里，上游侧同样是红的。
vi.mock("@/stores/auth", () => ({
  useAuthStore: () => ({
    isSimpleMode: false,
    user: null,
