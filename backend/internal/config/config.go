...
func setEnvReachableDefaults() {
	...
	// MERCHANT-SYSTEM 与 guard 上报是 fork 私有开关，运维常靠纯环境变量部署（无 config.yaml）。
	// 加这些功能时漏在此注册零值默认，viper 会静默丢弃对应 env（MERCHANT_ENABLED 等），
	// 效果等同当年 image_storage 凭证丢失。零值即现有隐式默认，此处只补 env 可达性、不改行为。
	viper.SetDefault("merchant.enabled", false)
	viper.SetDefault("merchant.server_ip", "")
	viper.SetDefault("merchant.skip_dns_verify", false)
	viper.SetDefault("merchant.affiliate_rebate_enabled", false)
	viper.SetDefault("gateway.guard_report.base_url", "")
	viper.SetDefault("gateway.guard_report.agent_token", "")
}
