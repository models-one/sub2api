...
// AllowedQuotaPlatforms 里的每个平台都必须能真正写进 user_platform_quotas。
// 历史 bug：国产平台不在 CHECK 约束内 → 注册预填充时整条多行 INSERT 中止 →
// fail-open 吞错 → 该用户一条已配置的默认限额都没落库（= 全平台无限额）。
//
// 名单按 service.AllowedQuotaPlatforms 派生而不是写死：parity 测试只能挡住
// 「三侧名单不同集」，挡不住「CHECK 语法写对了但 INSERT 仍失败」这一类；
// 而写死名单会在每次新增平台时静默漏掉它（0.2.5 的 opencode_go 就是这么漏的）。
// 每条记录都带 DailyLimitUSD：0.2.5 起三档全空的记录会被 configuredRecords 过滤掉、根本不落库。
func TestUserPlatformQuotaRepository_BulkInsertInitial_CNProvidersAllowed(t *testing.T) {
	...
	for _, p := range service.AllowedQuotaPlatforms {
		records = append(records, UserPlatformQuotaRecord{UserID: userID, Platform: p, DailyLimitUSD: &daily})
	}
	require.NoError(t, repo.BulkInsertInitial(txCtx, records),
		"AllowedQuotaPlatforms 里的全部平台都应可写入（DB CHECK 必须覆盖它们）")
