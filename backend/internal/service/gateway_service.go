...
// 仅当：① streamResult 非 nil 且 usage 确实观测到非零 token；② firstTokenMs 非 nil
// （确实已向客户端交付过内容）——此时上游已产出可计费 token、内容不可撤销且禁止 failover，
// 应计费。出错发生在产出任何内容之前（firstTokenMs == nil）、streamResult 为 nil（如读
// 错误触发的 failover）、或全程只收到 ping 之类无 usage 的事件时返回 false，不计费、
// 交由上层 failover/报错。
//
// hasObservedTokens 是上游 issue #5148 引入的判据，本 helper 一并采用：原先只判
// usage 指针非空，指针非空但值全零（例如只收到 ping 就断流）会产生一条零 usage 的
// 幽灵账单记录。改判之后本地部分计费能力不变（已交付内容仍照常计费），只是不再落
// 零值幽灵记录，与上游 partialStreamUsageResult 口径一致。
//
// 本地守卫（非上游代码）：Forward / forwardAnthropicAPIKeyPassthroughWithInput 的
// PartialError 部分计费路径依赖本 helper（streamingResult 定义已随上游重构移至
// gateway_upstream_response.go，同包可直接引用）。
func shouldBillPartialStream(streamResult *streamingResult) bool {
	return streamResult != nil && streamResult.usage.hasObservedTokens() && streamResult.firstTokenMs != nil
}
