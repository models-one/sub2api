	...
	for {
		...
		if reqStream {
			...
			if handleErr != nil {
				...
				// fork 定制（合并上游时注意保留）：流式已向客户端交付内容后中途出错，
				// 返回带 PartialError 的结果（携带已产出 usage）连同原始 error，由上层对
				// 已交付 token 计费且按失败处理；否则整段零计费。
				// failover 错误不走部分计费——handler 会换账号重试，重试成功后按成功口径
				// 计费，此处计费会造成双重入账（且 failover 仅在未交付内容时返回，本就不满足
				// 计费条件）。
				// 位置：必须排在上游 0.1.179 新增的 compact fallback 重试/信号两条分支**之后**、
				// 兜底 `return nil, handleErr` **之前**——那两条分支只在尚未产出可见输出时触发
				// （shouldBillPartialOpenAIPassthroughStream 要求 firstTokenMs != nil），
				// 放在它们之前会把可重试的请求变成一次部分计费的失败。
				var failoverErr *UpstreamFailoverError
				if shouldBillPartialOpenAIPassthroughStream(result) && !errors.As(handleErr, &failoverErr) {
					...
					_ = resp.Body.Close()
					return partialResult, handleErr
				}
				_ = resp.Body.Close()
...
func (s *OpenAIGatewayService) handleErrorResponsePassthrough(
	...
	// 脱敏后的上游消息，而不是抹成通用文案。
	// 本地错误脱敏（去除上游渠道身份 host/url/key/IP）由上游的净化信封收口：错误体
	// 一律本地重建、不再原样回写上游 body，强度高于原先的「脱敏后透传」；
	// context-window 分支回带的 upstreamMsg 也已过 sanitizeUpstreamErrorMessage。
	// 此处不得再补 c.Data 回写上游 body，否则既重复写响应又会重新泄漏渠道身份。
