...
type postUsageBillingParams struct {
	...
	// 本 fork 补充：该模式下同样不得产生商户分润 / owner ledger / 邀请返利等任何资金副作用，
	// buildUsageBillingCommand 的 simple 短路在透传上面五个 fork 字段之前就 return。
...
func buildUsageBillingCommand(requestID string, usageLog *UsageLog, p *postUsageBillingParams) *UsageBillingCommand {
	...
	if p.SimpleModeKeyRateLimitOnly {
		// simple 模式只记 API-key 5h/1d/7d 窗口用量（按 base ActualCost，与下方
		// 「rate_limit 永远用 base」口径一致）。必须在透传 BalanceCostOverride /
		// MerchantOutbox / MerchantLedger / AffiliateConsumeOutbox /
		// MerchantAffiliateConsumeOutbox 之前 return：simple 模式不得动钱（fork 定制守卫）。
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput) error {
	...
	// token 倍率叠加高峰因子（token 计费含图片 token，图片按次倍率不受影响）。高峰因子按请求级
	// PricingAt 现算（与利润门 D 同源同刻，跨峰谷请求不中途变价）；未装配 PricingAt 的路径回退
	// 记录时刻，保持既有行为。不并入上面的 getUserGroupRateMultiplier，以免污染 user:group 倍率缓存。
	// account 入参是本地守卫：影子账号不吃高峰倍率，恒按母账号继承倍率走（见 computePeakAwareMultipliers）。
	pricingAt := input.PricingAt
	if pricingAt.IsZero() {
		pricingAt = timezone.Now()
	}
	multiplier, imageMultiplier := computePeakAwareMultipliers(apiKey, account, multiplier, pricingAt)
	...
	// （MERCHANT-SYSTEM 生命线：合并上游时只跟随签名变化——本轮上游删掉了
	// buildRecordUsageLog 的 opts *recordUsageOpts 形参，因为旧长上下文规则整体收口进
	// 价格目录——这段 override 本身不动。）
