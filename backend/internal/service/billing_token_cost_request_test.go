...
// fork 加固（保留）：原 TestLegacyLongContextRule_OnlyGemini 守的是
// BillingService.LegacyLongContextRule —— 平台级硬编码「>200K 部分 ×2.0」旧规则。
// 上游 0.1.185 把长上下文阶梯整体收口进价格目录数据（above_XXXk 折算），
// LegacyLongContextRule / legacyLongContextApplies / TokenCostRequest.LegacyLongContext
// 全部删除，那个断言已无对象可断。
//
// 但它防的失效场景仍然成立、且只是换了入口：**某个平台在没有目录阶梯数据时
// 仍然凭平台身份被套上长上下文倍率 ⇒ >200K 请求被静默乘 2.0，属直接多收且无告警**。
// 因此把守卫平移到现在唯一的入口（CalculateTokenCostForRequest + 无阶梯字段的目录条目），
// 并保留上一轮补齐的完整反例平台清单：本 fork 终态 8 平台全覆盖（上游只列自己关心的几个，
// 漏掉 grok 与改名后的国产三家 kimi/zhipu/deepseek），外加空平台。
// 若日后有人重新引入按平台判定的阶梯，本用例会在对应平台上失败。
func TestCalculateTokenCostForRequest_NoPlatformLevelLegacyLongContextRule(t *testing.T) {
	group := &Group{ID: 100, LongContextPricingEnabled: true}
	gid := group.ID
	tokens := UsageTokens{InputTokens: 300000, OutputTokens: 1000}

	for _, platform := range []string{
		PlatformGemini, PlatformOpenAI, PlatformAnthropic, PlatformAntigravity, PlatformComposite,
		PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek, "",
	} {
		bs, resolver := newTokenCostTestEnv(t, platform, nil, geminiCatalogStub())
		...
		// 300K × 1.25e-6 + 1000 × 10e-6 = 0.385（无任何倍率）
		require.InDelta(t, 0.385, got.ActualCost, 1e-9, platform)
		require.False(t, got.LongContextBillingApplied, platform)
	}
}
