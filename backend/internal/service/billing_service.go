...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// 分组开关是统一入口；账号级开关是**双向**的：显式 true 可加开（上游 0.1.179 新增的能力，
	// 分组关时也能单账号开），显式 false 可否决（fork 既有语义，必须保留）。
	//
	// ⚠️ 合并上游时注意保留：上游此处写的是「只加开、false 不否决」
	// （`if input.LongContextBillingEnabled != nil && *input.LongContextBillingEnabled`）。
	// 直接跟随会在本 fork 生产上全线多收并让后台开关变成摆设，因为两侧默认值正好相反：
	//   - groups.long_context_pricing_enabled 在迁移 221 是 NOT NULL DEFAULT TRUE，
	//     且该迁移把**存量分组全部刷成 TRUE**；
	//   - OpenAI 账号的 openai_long_context_billing_enabled 在 admin_account.go 的
	//     normalize 里缺省写 false，IsOpenAILongContextBillingEnabled() 也只在显式 true 时为真。
	// 「分组恒 true × 账号默认 false」正是绝大多数生产账号的组合：按上游语义 applyLongCtx
	// 会恒为 true，等于把 2026-07-14 拍板的「OpenAI 长上下文计费默认关」整个推翻
	//（GPT-5.6 超 272K 阈值 input×2.0 / output×1.5），且管理员把账号开关关掉也不再生效。
	// openAILongContextBillingGate() 之所以返回 *bool 而不是 bool（并注释「非 OpenAI 平台
	// 返回 nil 而不是硬编码 false，否则会否决官方模型阶梯」）也只有在否决语义下才讲得通。
	contextTierPricingEnabled := resolved.longContextPricingEnabled
	if input.LongContextBillingEnabled != nil {
		contextTierPricingEnabled = *input.LongContextBillingEnabled
	}
	...
	// 上游把这里从 `resolved.longContextPricingEnabled + 账号 AND` 改成统一读
	// contextTierPricingEnabled，本 fork 跟随该重构（同一个开关同时决定 pricingContext
	// 塌档与阶梯应用，避免两处口径漂移），账号开关的否决语义收敛在上面的赋值处。
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool, pricingAt time.Time) *ModelPricing {
	...
	// fork 定制（保留）：Gemini 走独立分支补「缓存写入价 = 标准输入价」后直接返回，
	// 不进下面的 OpenAI 专属策略。与上游新增的 DeepSeek 强制官方价互不相交（模型族不同），
	// 放在最前只是维持本 fork 既有形状；合并上游时不要把这段吞掉。
	if isGeminiModel(model) {
		return fillGeminiCacheWritePrice(pricing)
	}
	...
	// 注：上游 0.1.185 把长上下文阶梯整体收口进价格目录（above_XXXk 绝对价由解析层折算成
	// 阈值+倍率），本函数不再给 GPT-5.4/5.5/5.5-pro/5.6 兜底补阶梯，
	// openAIGPT54LongContext* 常量与 usesOpenAILegacyLongContextPricing 名单一并删除。
	// 本 fork 跟随该口径（避免与目录阶梯双重叠加）；代价是静态兜底价卡
	// （日志「Using fallback pricing for model: gpt-5.x」）在目录缺该模型时不再有阶梯，
	// >272K 请求按基础价计——见本轮合并报告的 risk 项。
	// 本 fork 在这条链路上真正的定制是下面 calculateTokenCost 里的
	// stripLongContextPricing（只对 GPT-5.6 渠道 flat 约定价剥阶梯），未受影响。
	// 另：上游 0.2.8（b8d52fad3）把单一 max 推理倍率换成按等级配置的
...
// isGeminiModel 判断模型是否属于 Gemini 系列（兼容 models/、publishers/google/models/、
// gemini/ 等前缀形态，取最后一段判断）。
func isGeminiModel(model string) bool {
	return strings.HasPrefix(lastSegment(strings.ToLower(model)), "gemini")
}

// fillGeminiCacheWritePrice 为 Gemini 补"缓存写入价=输入价"。
// LiteLLM 价表的 gemini 条目一律没有 cache_creation_input_token_cost——Google 的
// 缓存没有写入加价：隐式缓存写入按普通输入价计，显式缓存创建也是标准输入价
// （另有按小时存储费，不在 per-token 口径内）。不补的话模型列表"缓存创建价格"列
// 恒为空。仅对确实支持缓存（有缓存读取价）的型号补值；对实际计费无影响——
// Gemini 用量从不产生 CacheCreationTokens（extractGeminiUsage 只把
// cachedContentTokenCount 映射到 CacheReadInputTokens）。
func fillGeminiCacheWritePrice(pricing *ModelPricing) *ModelPricing {
	if pricing.CacheCreationPricePerToken > 0 || pricing.CacheReadPricePerToken <= 0 || pricing.InputPricePerToken <= 0 {
		return pricing
	}
	cloned := *pricing
...
// （或按 provider 过滤）。providerFilter 为空时返回所有 chat 模型。
