...
func (s *GatewayService) ForwardAsResponses(
	...
	var handleErr error
	if clientStream {
		result, handleErr = s.handleResponsesStreamingResponse(resp, c, originalModel, mappedModel, reasoningEffort, toolCtx, startTime, clientToolMapping)
	} else {
		result, handleErr = s.handleResponsesBufferedStreamingResponse(resp, c, originalModel, mappedModel, reasoningEffort, toolCtx, startTime, clientToolMapping)
	}
...
func (s *GatewayService) handleResponsesStreamingResponse(
	...
	// 注意：上游 adaptResponsesClientToolsForAnthropic 已在解析前把 custom/tool_search/
	// namespace 工具降级成 function，故此处 toolCtx 生产路径下恒为 nil，实际还原由下面的
	// clientToolRestorer 承担；两套机制不会重复还原。保留本地路径以兼容直接调用本函数的旧测试。
