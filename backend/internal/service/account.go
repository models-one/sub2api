...
// GetKimiBaseURL / GetZhipuBaseURL / GetDeepseekBaseURL 只服务于「未显式声明
// api_protocol 的存量国产账号」的原生 Anthropic 直通路径（forwardAnthropicDirect），
// 保留改名前的默认端点，使存量账号行为不变。新链路一律走 GetOpenAIBaseURL /
// GetCNProtocolBaseURL（按 account_mode 选 payg / coding 默认值）。
...
func (a *Account) GetZhipuBaseURL() string {
	if a == nil || a.Platform != PlatformZhipu {
		return ""
	}
	if a.Type == AccountTypeAPIKey {
		if baseURL := strings.TrimSpace(a.GetCredential("base_url")); baseURL != "" {
...
func (a *Account) SupportsOpenAIImageCapability(capability OpenAIImagesCapability) bool {
	...
	case OpenAIImagesCapabilityBasic, OpenAIImagesCapabilityNative:
		// 仅 OpenAI 平台支持 image 功能（平台闸门在函数入口，已由上方 IsOpenAI 守卫）。
