	...
	if resp.StatusCode >= 400 {
		...
		if s.rateLimitService != nil {
			...
			case ErrorPolicySkipped:
				...
				// 错误响应不裸透传上游 body（会暴露上游厂商/中转域名），改为脱敏后的错误结构。
				...
				if account.IsCustomErrorCodesEnabled() {
					// 自定义错误码未命中且不可 failover：客户端只拿固定文案，上游细节仅进 ops 日志。
				...
				// 池模式：状态码保真（下游据此区分请求级错误与可重试的链路故障），
				// 但不裸透传上游 body —— 重新按 Google 错误结构写出脱敏后的 message，
				// 与 ForwardAsChatCompletions 的池模式写出口径一致。
				_ = s.writeGoogleError(c, resp.StatusCode, skipMsg)
		...
		// 本 fork 定制：状态码保真，但不裸透传上游 body（避免暴露上游厂商/中转域名），
		// 改为脱敏后的错误信息。上游此处走 writeGeminiNativeUpstreamError（body 保真）。
		errMsg := upstreamMsg
		if errMsg == "" {
			errMsg = "Upstream request failed"
		}
		_ = s.writeGoogleError(c, resp.StatusCode, errMsg)
		if upstreamMsg == "" {
	...
	if stream {
		...
		if err != nil {
			...
			if shouldBillPartialGeminiNativeStream(streamRes) {
				// ReasoningEffort 与下方正常路径同源（上游 e47255715），否则中断流的部分计费
				// 会跳过渠道 reasoning_effort_multipliers，与正常完成的请求口径不一致。
				return &ForwardResult{
					RequestID:       requestID,
					Usage:           *streamRes.usage,
					Model:           originalModel,
...
// 与主路径 shouldBillPartialStream 同口径：仅当 streamRes 非 nil 且携带 usage、firstTokenMs
// 非 nil（确实已向客户端交付过内容）时才计费；否则不计费、保留上层 failover 语义。
...
// usage、firstTokenMs 非 nil（确实已向客户端交付过内容）时才计费；否则不计费、保留上层
// failover 语义。
