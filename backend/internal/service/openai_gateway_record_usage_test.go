...
// 由上游 0.1.179 的 ..._GroupOrAccountLongContextAllows 改写。
//
// 上游语义是「分组或账号任一为真即生效」(OR)；本 fork 是「账号显式设置时**双向**覆盖分组，
// 未设置(nil，即非 OpenAI 平台)时完全跟随分组」。差异只落在 group on × account off 这一格：
// 上游算生效，fork 算被否决。而这一格正是生产上绝大多数账号的组合——
// 分组侧 groups.long_context_pricing_enabled 被迁移 221 建成 NOT NULL DEFAULT TRUE 并把
// 存量分组全刷成 TRUE，账号侧 openai_long_context_billing_enabled 在 admin_account.go 的
// normalize 里缺省写 false。按上游语义，一笔 gpt-5.4 300k input + 2k output 的请求会
// input ×2、output ×1.5 凭空多收约 92%，且管理员在后台把账号开关关掉也不再生效，
// 直接推翻 2026-07-14 拍板的「OpenAI 长上下文计费默认关」。
// 口径若要改成上游的 OR，须由商务/站长显式拍板，不能靠一次合并静默切换。
func TestOpenAIGatewayServiceRecordUsage_AccountLongContextGateOverridesGroup(t *testing.T) {
	tokens := OpenAIUsage{InputTokens: 300000, OutputTokens: 2000}
	baseInput := 300000 * 2.5e-6
	baseOutput := 2000 * 15e-6

	t.Run("group on account off vetoes", func(t *testing.T) {
		usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
		svc := newOpenAIRecordUsageServiceForTest(usageRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)
		swapInOpenAILadderCatalog(t, svc)
		err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		...
		require.NoError(t, err)
		// 账号 Extra 里没有 openai_long_context_billing_enabled → IsOpenAILongContextBillingEnabled()
		// 缺省 false，openAILongContextBillingGate 返回 &false → 否决分组的 true，按标准价计费。
		require.False(t, usageRepo.lastLog.LongContextBillingApplied,
			"账号级开关为 false 时必须否决分组开关，否则线上绝大多数 OpenAI 账号会凭空多收")
...
// 回归：free_openai_fast 的标准价重算必须继承 fork 既有的「价卡缺失 → 零成本落账」容错。
//
// 上游 0.1.185 引入 free_openai_fast 时，重算失败是直接 `return standardErr`。但
// writeUsageLogBestEffort 排在该分支之后，提前 return 意味着 usage_log 一行都不写、
// 不扣费、也不进 GuardReporter —— 对开了该开关的分组，遇到价卡缺失的模型（含
// filterCNProviderBillingModelCandidates 把候选全过滤掉的 CN 账号场景）会从本 fork 既有的
// 「零成本 + pricing_missing_record_zero_cost 告警落账」退化成**彻底静默丢单**。
// calculateOpenAIRecordUsageCost 是纯函数、两次调用入参相同，所以第一次命中价卡缺失时
// 重算必然复现同一个错误，这是可预期路径而不是异常。
//
// 本用例构造「无任何价卡的模型 + FreeOpenAIFast=true」：断言 RecordUsage 不返回错误，
// 且 usage_log 仍然落账（零成本）。去掉容错分支后本用例立即变红。
func TestOpenAIGatewayServiceRecordUsage_FreeOpenAIFastKeepsZeroCostLogWhenPricingMissing(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	svc := newOpenAIRecordUsageServiceForTest(
		usageRepo,
		&openAIRecordUsageUserRepoStub{},
	...
	apiKey := &APIKey{
		...
		Group: &Group{
			...
			// 刻意不配任何 ModelPricing —— 该模型在价目表与兜底表里都查不到价。
		},
	}

	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:                   "resp_free_fast_no_pricing",
			ServiceTier:                 &serviceTier,
			UpstreamResponseServiceTier: "default",
			Usage:                       OpenAIUsage{InputTokens: 100, OutputTokens: 50},
			Model:                       "totally-unknown-model-with-no-pricing-9x7",
			Duration:                    time.Second,
		},
		APIKey:  apiKey,
		User:    &User{ID: 2021},
	...
	require.NoError(t, err, "价卡缺失不能让 RecordUsage 整条失败——那会连审计行都丢掉")
	require.NotNil(t, usageRepo.lastLog, "必须仍然落一条零成本 usage_log")
	require.InDelta(t, 0, usageRepo.lastLog.TotalCost, 1e-12)
	require.InDelta(t, 0, usageRepo.lastLog.ActualCost, 1e-12)
}
