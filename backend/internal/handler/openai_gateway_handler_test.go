	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gatewaySvc := service.NewOpenAIGatewayService(
				...
				nil,      // merchantPricing（本地 fork）
				nil,      // affiliateRebatePricing（本地 fork）
				nil, nil, // merchantAffiliateRebate（本地 fork）, userPlatformQuotaRepo
			)
			h := NewOpenAIGatewayHandler(
				gatewaySvc,
				service.NewConcurrencyService(nil),
...
func TestOpenAIResponses_APIKeyPassthroughSSERateLimitUsesConfiguredPoolRetry(t *testing.T) {
	...
	gatewaySvc := service.NewOpenAIGatewayService(
		...
		nil,      // merchantPricing（本地 fork）
		nil,      // affiliateRebatePricing（本地 fork）
		nil, nil, // merchantAffiliateRebate / userPlatformQuotaRepo
	)
	h := NewOpenAIGatewayHandler(
		gatewaySvc,
		service.NewConcurrencyService(nil),
...
func TestOpenAIResponsesWebSocket_FirstOutputTimeoutWithoutDownstreamReusesClientForOneFailover(t *testing.T) {
	...
	gatewaySvc := service.NewOpenAIGatewayService(
		...
		nil, nil, nil, // 本地 fork 新增：merchantPricing 之后的 affiliateRebatePricing / merchantAffiliateRebate / userPlatformQuotaRepo，测试传 nil
	)
	cache := &concurrencyCacheMock{
