...
func TestGeminiNativeModelsMergesNativeUpstream(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			h.GeminiV1BetaListModels(c)
			if tt.status == 429 {
				// 上游这条用例断言「上游错误体原样透传」（require.JSONEq(tt.body, ...)），
				// 与本 fork 有意的分歧冲突：fork 在 gemini_v1beta_handler.go 的
				// writeUpstreamResponse 里加了 `res.StatusCode >= 400` 分支，
				// 只保真状态码，**不裸透传上游 body 与响应头**，改写成脱敏后的
				// Gemini 原生错误结构，以免把上游厂商/中转域名暴露给客户端
				// （上游该函数没有这个分支，4xx/5xx 一律 c.Data 原样吐出）。
				// 因此这里断言 fork 的真实契约：状态码保真 + 上游原文不外泄。
				require.Equal(t, 429, rec.Code)
				require.NotContains(t, rec.Body.String(), "rate limited",
					"上游错误体不得原样透传给客户端（会暴露上游厂商/中转域名）")
				...
				require.Equal(t, 429, errEnvelope.Error.Code)
				require.NotEmpty(t, errEnvelope.Error.Message)
				return
			}
