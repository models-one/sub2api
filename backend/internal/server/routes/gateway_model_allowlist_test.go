...
func newGatewayRoutesTestRouterWithGroup(group *service.Group) *gin.Engine {
	...
	RegisterGatewayRoutes(
		...
		nil, // 本 fork 在 cfg 之后追加的 merchantRepo（MERCHANT-SYSTEM 停用商户守卫）
	)
	return router
}
...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	// 注意：本 fork 的 APIKeyAuthWithSubscriptionGoogle 比上游多一个 merchantRepo 入参
	// （MERCHANT-SYSTEM 停用商户守卫），而这是一条**按源码文本正则**断言的用例，
	// 所以两处 auth 字面量必须带上 merchantRepo，否则恒不匹配。
	// 用例真正要守的语义（groupModelAllowlist 挂在 auth 之后、composite 之前）不变。
	chains := []struct {
		group     string
		auth      string
		marker    string
		...
		{group: "gemini", auth: "middleware.APIKeyAuthWithSubscriptionGoogle(apiKeyService, subscriptionService, cfg, merchantRepo)", marker: "gemini.Use(groupModelAllowlist)", composite: "gemini.Use(compositeGeminiTarget)"},
		...
		{group: "antigravityV1Beta", auth: "middleware.APIKeyAuthWithSubscriptionGoogle(apiKeyService, subscriptionService, cfg, merchantRepo)", marker: "antigravityV1Beta.Use(groupModelAllowlist)", composite: "antigravityV1Beta.Use(requireGroupGoogle)"},
	}
