...
func RegisterGatewayRoutes(
	...
	countTokensHandler := func(c *gin.Context) {
		...
		default:
			...
			// unsupported response instead of falling back to the Anthropic path.
	...
	imagesHandler := func(c *gin.Context) {
		...
		case supportsImageGenEndpoint(platform):
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		// locally, other OpenAI-compatible (domestic) platforms return the
		// unsupported response, and Anthropic-compatible platforms retain their
		// existing path (all handled by countTokensHandler).
		...
		gateway.POST("/embeddings", textBodyLimit, func(c *gin.Context) {
			// 本地放宽：国产 OpenAI 兼容平台（Kimi/Zhipu/DeepSeek/MiniMax）同样支持 embeddings
			if !isOpenAICompatPlatform(getGroupPlatform(c)) {
				service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
				c.JSON(http.StatusNotFound, gin.H{
					"error": gin.H{
	...
	// MERCHANT-SYSTEM v1.0: merchantRepo 供停用商户守卫使用，必须保留。
...
// gateway (OpenAI plus the domestic providers Kimi / Zhipu / Deepseek / MiniMax).
// These platforms share the same /v1/chat/completions and /v1/responses handler.
// Grok has its own route family and is deliberately excluded.
//
// 新增国产 OpenAI 兼容供应商时同步这里（对齐 service.IsCNProvider 的名单）。
