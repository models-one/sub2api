...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	merchantRepository := repository.NewMerchantRepository(client, db)
	merchantPricingService := service.NewMerchantPricingService(configConfig, merchantRepository, userRepository)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, merchantPricingService, concurrencyService)
	...
	merchantAffiliateRepository := repository.NewMerchantAffiliateRepository(client, db)
	merchantAffiliateRebateService := service.NewMerchantAffiliateRebateService(configConfig, merchantAffiliateRepository)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, merchantAffiliateRebateService, serviceUserPlatformQuotaRepository)
	...
	userHandler := handler.NewUserHandler(userService, authService, emailService, emailCache, affiliateService, merchantAffiliateRebateService, serviceUserPlatformQuotaRepository)
	...
	openAIGatewayService := service.NewOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, merchantPricingService, affiliateRebatePricingService, merchantAffiliateRebateService, serviceUserPlatformQuotaRepository)
	...
	groupHandler := admin.NewGroupHandlerWithConfig(adminService, dashboardService, groupCapacityService, channelService, billingService, configConfig)
	...
	merchantOutboxRepository := repository.NewMerchantOutboxRepository(client, db)
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, configConfig, merchantRepository, merchantOutboxRepository, notificationEmailService)
	...
	merchantDomainRepository := repository.NewMerchantDomainRepository(client)
	...
	merchantGroupCostRepository := repository.NewMerchantGroupCostRepository(client)
	merchantService := service.NewMerchantService(configConfig, client, merchantRepository, merchantDomainRepository, merchantLedgerRepository, merchantAuditLogRepository, merchantGroupMarkupRepository, merchantGroupCostRepository, groupRepository, userRepository, merchantPricingService)
	merchantHandler := admin.NewMerchantHandler(merchantService)
	profitService := service.NewProfitService(client)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, modelPricingHandler, promptAdminHandler, paymentHandler, affiliateHandler, merchantHandler, profitHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	handlerMerchantHandler := handler.NewMerchantHandler(merchantService, userService, merchantAffiliateRebateService)
	...
	batchImagePublicService := service.NewBatchImagePublicService(batchImageRepository, accountRepository, groupRepository, userGroupRateRepository, batchImageQueue, batchImageModelPricingResolver, usageBillingRepository, apiKeyAuthCacheInvalidator, billingCacheService, configConfig)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, merchantBrandHandler, handlerMerchantHandler, merchantLogoHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	optionalJWTAuthMiddleware := middleware.NewOptionalJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService, auditLogService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig, merchantRepository)
	...
	batchImageWorkerRuntime := service.ProvideBatchImageWorkerRuntime(batchImageRepository, accountRepository, batchImageQueue, usageBillingRepository, usageLogRepository, batchImageModelPricingResolver, apiKeyAuthCacheInvalidator, billingCacheService, apiKeyService, configConfig)
	...
	merchantEarningsWorker := service.ProvideMerchantEarningsWorker(configConfig, db, merchantOutboxRepository)
	merchantReconcileJob := service.ProvideMerchantReconcileJob(configConfig, db, merchantOutboxRepository, paymentService)
	affiliateConsumeOutboxRepository := repository.NewAffiliateConsumeOutboxRepository(client, db)
	affiliateRebateWorker := service.ProvideAffiliateRebateWorker(configConfig, db, affiliateConsumeOutboxRepository, settingService)
	merchantAffiliateOutboxRepository := repository.NewMerchantAffiliateOutboxRepository(client, db)
	merchantAffiliateRebateWorker := service.ProvideMerchantAffiliateRebateWorker(configConfig, db, merchantAffiliateOutboxRepository)
	userPlatformQuotaUsageFlusher := service.ProvideUserPlatformQuotaUsageFlusher(configConfig, billingCache, serviceUserPlatformQuotaRepository, timingWheelService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, openAICodexVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, channelMonitorV2Aggregator, merchantEarningsWorker, merchantReconcileJob, affiliateRebateWorker, merchantAffiliateRebateWorker, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, auditLogService, openAIQuotaAutoResetService, promptService, pluginManager)
